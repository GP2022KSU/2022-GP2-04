...
class _OffersListState extends State<OffersList> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [DisplayWeidgtInOffer()],
        ),
      ),
    );
  }

  Widget DisplayWeidgtInOffer() {
    if (user.getPurchaseHistory().length > 3) {
      return RecommndedOffers();
    } else if (user.getNumofOffer() > 0) {
      return NormalOffer();
    }
    return Column(
          ...
          child: Container(
            ...
            decoration: const BoxDecoration(
              image: DecorationImage(
                  image: AssetImage('assets/images/offerPhoto.png'),
                  fit: BoxFit.fitWidth),
            ),
          ),
        ),
  ...
  Widget RecommndedOffers() {
    final ref = fb.ref().child('Products');
    return FutureBuilder<List<String>>(
        future: _SeeAPI(),
        builder: (BuildContext context, AsyncSnapshot<List<String>> asyn) {
      ...
      child: FirebaseAnimatedList(
        ...
        itemBuilder: (context, snapshot, animation, index) {
          ...
          try {
            ...
            price = double.parse(map['Price'].toString());
            offerprice = double.parse(map['PriceAfterOffer'].toString());
          } on Exception {}
  ...
  Future<List<String>> _SeeAPI() async {
    //run python file
    final url = 'http://10.6.196.47:5000/name'; //local python API
    var purchasehis = "";
